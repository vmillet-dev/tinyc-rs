# TinyC — architecture and language reference

How the compiler is put together, stage by stage, and every rule of the language
it accepts. For what the project is and how to run it, see the
[README](../README.md).

TinyC is a small compiler, written in Rust 2024 with no dependencies except
`clap`, that turns a tiny typed language into x86-64 assembly.
...
### The words are written down once

The grammar above is prose, but the **vocabulary** is not. The
[IntelliJ plugin](../ide/intellij/README.md) cannot run `tinyc` on every
keystroke, so it needs the words for itself; it used to have a *copy*, and a
copy is a place to forget. `cargo run --bin export-vocabulary` writes them out
to `grammar/vocabulary.txt` instead, and the plugin's token table is generated
from that file when it builds.

```text
src/token.rs ─┐
              ├─> grammar/vocabulary.txt ──> ide/intellij/…/TinyCTokens.kt
src/ast.rs   ─┘        checked in,               generated by Gradle
                    compared by a test
```

**The split is the design.** `token.rs` says what a token *is*, and
`TokenKind::text` is the one place a spelling is written — that file looks
exactly as it did before any of this existed. `vocabulary.rs` says what a token
is *for*: `Role`, which is what an editor colours by, and the name the Kotlin
field is generated from. Neither belongs in the compiler's own types, because a
parser matches on a `TokenKind` and never asks whether `if` is a keyword or
`int` a type name, and `"IF_KW"` is a Kotlin field name.

**No spelling is written twice.** A row in `vocabulary::SPELLED` names a
`TokenKind` and reads its spelling from `TokenKind::text`, so the two cannot
disagree. The lexer looks a word up in that same table rather than matching on
it, which is what makes a word left out of it not a keyword the editor forgot
but no keyword at all. `true` and `false` are the one exception, and they say so
in the table: they carry a value *and* are written one way, so `text` answers
"boolean literal" — right for "expected a boolean literal", useless for lexing.

What a table cannot promise is that nothing is *missing* from it, since Rust
cannot be asked what an enum's variants are. That is a test's job, and it is the
same bargain `token.rs` already made before: its tests kept a written-out list
of every token with a spelling, for exactly this reason. `SPELLED` is that list,
given roles and a use.

**Nothing in the exporter is a list.** Every loop in `export` walks a table that
lives beside the thing it describes — `SPELLED`, `Role::ALL`,
`ast::Builtin::ALL`, `ast::SPECS` — so growing the language means adding a row
where the language is defined and never opening the exporter. Two things make
that hold rather than merely be true today:

* A built-in states its signature in `Prim`s rather than `Ty`s. It is a name in
  the signature table before any program exists, so it cannot mention a class or
  an enum, and an array or a list is an interned id meaningless without a
  `TypeTable`. Saying that in the *type* means the exporter has no table to want
  and no case it can fail on — where it used to carry a `match` that panicked.
* `Role::is_word` is a `match` and not a `matches!`. A role missing from the
  word side of a `matches!` is silently `false`, which would make a new keyword
  lex as an identifier; written as a match, adding a `Role` does not compile
  until which of the two it is has been decided.

The plugin's generator is the same shape. It builds a set and a word list per
role it *finds in the file* rather than knowing what the roles are, so a role
added to the compiler arrives with `<ROLE>_TOKENS`, a place in `ALL_KEYWORDS`
and an entry in `ROLES` without a line written on that side.

The file is checked in so that building the plugin needs no Rust toolchain, and
`vocabulary::tests` compares it against what this compiler would write — so a
stale export is a failing `cargo test` rather than a word that quietly stops
being highlighted in an editor nobody thought to rebuild.

What is deliberately **not** generated is the parser. The plugin's is tolerant
on purpose: it looks only for declarations and lets anything it does not
recognise pass, which is what keeps it useful in a file being typed into
halfway through a word. A generated parser would reject instead, and would need
teaching every time the grammar above grew a line.
